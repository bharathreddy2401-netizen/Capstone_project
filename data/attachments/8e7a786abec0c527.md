...
- table:
  - rowgroup:
    - row "Item Description Price Quantity Total":
      ...
      - cell
  - rowgroup:
    - row "Product Image Stylish Dress Women > Dress Rs. 1500 5 Rs. 7500 ":
      - cell "Product Image":
        - link "Product Image":
          - /url: ""
          - img "Product Image"
...
```ts
  ...
  89 |         await cart.continue();
  90 |         await page.waitForTimeout(1000);
  91 |         await cart.cartbtn();
  92 |         await page.waitForTimeout(1000);
> 93 |         await expect(page.locator("#product-4 td button.dissabled")).toHaveText("15");
     |                                                                      ^ Error: expect(locator).toHaveText(expected) failed
  94 |     });
  95 |     
  96 | })
