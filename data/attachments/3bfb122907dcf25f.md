...
Call log:
  - waiting for locator('.product-text-indent').filter({ hasText: 'Men Tshirt' }).locator('.add-to-cart').first()
...
```ts
  ...
  7  |         this.Tshirt = page.locator('.product-text-indent', { hasText: 'Men Tshirt' }).locator('.add-to-cart').first();        
